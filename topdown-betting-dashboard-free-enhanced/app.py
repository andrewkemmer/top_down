...
# ---- Cache API fetch to prevent rerun resets ----
...
# ---- Session state for sportsbook filter ----
...
if fetch:
    if not api_key:
        ...
        try:
            ...
            # ---- Build sportsbook filter options once across all sports ----
            all_books = list({bmk['key'] for ev_list in cached_events_all_sports.values() for ev in ev_list for bmk in ev.get("bookmakers", [])})
            # Ensure defaults exist in options
            default_books = [b for b in st.session_state.selected_books if b in all_books] or all_books
            st.session_state.selected_books = st.sidebar.multiselect(
            ...
            # ---- Display results ----
            if not all_rows:
                st.warning("No rows generated — check sport/market/region/filters.")
            else:
