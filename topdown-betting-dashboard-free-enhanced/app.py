...
# List of all supported player prop market codes
supported_props = list(prop_markets.values())
...
# Game line mapping
market_label_to_code = {"Moneyline (H2H)": "h2h", "Spreads": "spreads", "Totals": "totals"}
    ...
    if not api_key:
        ...
        try:
            for sport_name in selected_sports:
                ...
                # Skip unsupported player props for this sport
                if market_category == "Player Props" and market_code not in supported_props:
                    st.warning(f"{prop_label} is not supported for {sport_name} by The Odds API. Skipping.")
                    continue
