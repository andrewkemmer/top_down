...
def fetch_odds_cached(api_key, sport_slug, region, market_code):
    return fetch_odds_theoddsapi(api_key, sport_slug, region, market_code)
...
if api_key:
    try:
        ...
        selected_sports = st.sidebar.multiselect("Select sport(s)", list(sport_options.keys()), default=["NBA"])
...
# ---- Odds filter ----
min_american = st.sidebar.number_input("Min American odds", value=-1000)
max_american = st.sidebar.number_input("Max American odds", value=5000)
min_decimal = st.sidebar.number_input("Min Decimal odds", value=1.01)
max_decimal = st.sidebar.number_input("Max Decimal odds", value=100.0)

# ---- Session state for sportsbook filter handled by Streamlit key ----
if "cached_events_all_sports" not in st.session_state:
    st.session_state.cached_events_all_sports = {}
...
if fetch:
    if not api_key:
        ...
        try:
            ...
            # Fetch events per sport and store in session state
            cached_events_all_sports = {}
            for sport in selected_sports:
                sport_slug = sport_options[sport]
                raw_events = fetch_odds_cached(api_key, sport_slug, region, market_code)
                cached_events_all_sports[sport] = raw_events
                st.sidebar.write(f"{sport}: {len(raw_events)} events fetched")
            st.session_state.cached_events_all_sports = cached_events_all_sports
...
# ---- Build combined sportsbook list ----
cached_events_all_sports = st.session_state.get("cached_events_all_sports", {})
all_books = list({bmk['key'] for sport_events in cached_events_all_sports.values() 
                 for ev in sport_events for bmk in ev.get("bookmakers", [])})

# ---- Sportsbook multiselect using key (no overwriting) ----
if all_books:
...
selected_books = st.session_state.get("selected_books", all_books)

# ---- Process events and compute EV/edge ----
all_rows = []
for sport, raw_events in cached_events_all_sports.items():
    for ev in raw_events:
        ...
        for bk, od in per_book_odds.items():
            ...
            for team in od.keys():
                ...
                # Apply odds filters
                if not (min_american <= od[team] <= max_american and min_decimal <= dec <= max_decimal):
                    continue
